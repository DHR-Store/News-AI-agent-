...
async function fetchNews(loadMore = false) {
    ...
    let apiUrl = `https://gnews.io/api/v4/search?q=${encodeURIComponent(topic)}&lang=en&country=us&max=5&page=${newsPage}&apikey=3628c12e5f5c5fb9a037739563d732ff`;
    ...
    try {
        ...
        data.articles.forEach(article => {
            let summary = summarizeText(article.description || article.content, 2);
            ...
            newsCard.innerHTML = `
                ...
                <button onclick="translateSummary('${summary}', 'hi')">🌍 Translate to Hindi</button>
            `;
...
// **News Summarizer (Extractive AI Approach)**
function summarizeText(text, sentenceCount = 2) {
    ...
    let wordFreq = {};
    let stopWords = new Set(["is", "the", "and", "to", "of", "in", "a", "that", "on", "for", "with", "as", "at", "by"]);
    ...
    let sentenceScores = sentences.map(sentence => {
        let words = sentence.toLowerCase().replace(/[^a-z\s]/g, "").split(/\s+/);
        let score = words.reduce((acc, word) => acc + (wordFreq[word] || 0), 0);
...
// **Social Media Sharing (Twitter Example)**
...
// **Infinite Scrolling (Auto Load More News)**
...
async function fetchNews() {
    ...
    let apiUrl = `https://gnews.io/api/v4/search?q=${encodeURIComponent(topic)}&lang=en&country=us&max=5&apikey=3628c12e5f5c5fb9a037739563d732ff`;
