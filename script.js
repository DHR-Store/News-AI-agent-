...
document.addEventListener('DOMContentLoaded', () => {
    let newsPage = 1;
    let videoPageToken = "";
    let currentTopic = "latest news"; // डिफ़ॉल्ट न्यूज़ टॉपिक
    let showingVideos = false;
    let isFetching = false;
    ...
    async function fetchNews(loadMore = false) {
        ...
        let apiUrl = `https://newsapi.org/v2/everything?q=${encodeURIComponent(currentTopic)}&page=${newsPage}&pageSize=5&apiKey=${NEWS_API_KEY}`;
    ...
    // **Auto Load News & Videos on Home Page**
    fetchNews(); // पेज लोड होते ही न्यूज़ लोड करें
    fetchVideos(); // वीडियो न्यूज़ भी लोड करें

    // **Event Listeners for Buttons**
    document.getElementById('fetch-news')?.addEventListener('click', () => {
        currentTopic = document.getElementById('news-topic').value.trim() || "latest news";
        fetchNews();
    });
